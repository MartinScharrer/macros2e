...
\def\exthref#1#2{{%
    \Gin@defaultbp\x{\zposx{ext-#1}sp}
    \Gin@defaultbp\y{\zposy{ext-#1}sp}
    \zrefused{ext-#1}
    \edef\page{\zref@extract{ext-#1}{abspage}}%
    \ifnum0\page=0\else
        \href[page=\page,pdfnewwindow=true,%
        pdfremotestartview=FitH \y]{file:\sourceIIe.pdf}{#2}%
    \fi
}}

\zexternaldocument[ext-]{\sourceIIe}

\let\origDescribeMacro\DescribeMacro
\def\DescribeMacro#1{%
  \llap{\expandafter\exthref\expandafter{\expandafter\@gobble\string#1}{LINK}~~~}%
  \origDescribeMacro{#1}%
}
...
\DescribeMacro\@ifdefineable!\@backslashchar!<name>{<yes>}
\noindent
Tests if \MacroArgs!\@backslashchar!<name> is undefined, \MacroArgs<name> not `|relax|'
and doesn't start with `|end|', and if \Macro{end}<name> is not defined. 

\DescribeMacro\@onlypreamlbe<macro>
\noindent
...
\DescribeMacro\g@addto@hook<macro>{<code>}
\noindent
